...
public class DbInit {
    ...
    public void init() {
        ...
        Restaurant restaurant2 = new Restaurant("반반스프링스 경상대점", "박승규",
                "52827, 경남 진주시 가좌안골길6번길 1 (가좌동), 1층 반반스프링스커피", point,30, FoodCategory.CAFE_DESSERT, RestaurantType.FOR_HERE_TO_GO, 10, 2);

        Restaurant restaurant3 = new Restaurant("abcabc", "ㄷㄷㄷ",
                "52828, 우 우리 우리집 501 (가좌동), 내방", point,10, FoodCategory.PIZZA, RestaurantType.FOR_HERE_TO_GO, 10, 2);

        restaurantService.registerRestaurant(restaurant1, byeongGyuId);
        restaurantService.registerRestaurant(restaurant2, seongGyuId);
        restaurantService.registerRestaurant(restaurant3, minjuId);
